...
public class SummonerCrossBow extends ProjectileWeaponItem implements Vanishable {
    ...
    public SummonerCrossBow(Properties properties) {
        super(properties);
    }

    @Override
    public Predicate<ItemStack> getAllSupportedProjectiles() {
        return (arg) -> arg.is(SummonerItems.SUMMON_ARROW.get());
    }
    ...
    private static boolean tryLoadProjectiles(LivingEntity livingEntity, ItemStack itemStack) {
        ...
        for(int k = 0; k < j; ++k) {
            ...
            addChargedProjectile(itemStack, SummonerItems.SUMMON_ARROW.get().getDefaultInstance());
        }
    ...
    public static boolean containsChargedProjectile(ItemStack itemStack, Item item) {
        return getChargedProjectiles(itemStack).stream().anyMatch((itemStackx) -> {
            return itemStackx.is(item);
        });
    }
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> list, TooltipFlag tooltipFlag) {
        ...
        if (isCharged(itemStack) && !list2.isEmpty()) {
            ...
            if (tooltipFlag.isAdvanced() && itemStack2.is(Items.FIREWORK_ROCKET)) {
                List<Component> list3 = Lists.newArrayList();
